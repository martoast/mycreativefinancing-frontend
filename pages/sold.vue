<template>
    <div class="bg-black min-h-screen">
      <div class="mx-auto max-w-2xl px-4 py-8 sm:px-6 sm:py-24 lg:max-w-7xl lg:px-8">
        ...
        <h1 class="text-2xl md:text-3xl lg:text-4xl font-extrabold tracking-wide mb-8 text-center">
          <span class="text-primary">SOLD</span> <span class="text-white">PROPERTIES</span>
        </h1>
  
        <!-- Link back to Available Properties -->
        <div class="mb-4 flex justify-end">
          <NuxtLink to="/" class="text-primary hover:text-indigo-300 font-semibold">
          ...
          >
            ...
            <div class="flex flex-1 flex-col p-4">
              ...
              <div class="mt-2 text-sm text-white">
                <span class="font-semibold text-red-700">Sold</span>
              </div>
  ...
  const { data, pending, error, refresh } = await useAsyncData(
    'sold-properties',
    () => store.get(currentPage.value, itemsPerPage, true) // Always fetch sold properties
  )
  ...
  const properties = computed(() => {
    return store.properties
      .filter(property => property.sold) // Only show sold properties
